...
README 的最终测试脚本另通过独立 4/4 复核，临时浅克隆 fixture 不依赖宿主 origin URL 或本地远端分支引用。任务 DONE 表示上述本机模块质量闭环，不意味着人工验收已完成。
...
- 同 run terminal 已只读核对：`PASS`、exit 0、elapsed `28800.74027900002` 秒，终态文件 SHA-256 为 `00BE7586AD05A8CDD03CFC510C7C730021920BEDB8EFDC1225E4A24765A398CD`。
- 连续性不是由 terminal 标签推断：4,952 个资源样本跨 `1.785503099905327`–`28800.721267899964` 秒，4,951 个相邻 gap 最大 `8.19159369985573` 秒，0 个超过 10/15/30 秒；完整第 0–7 小时样本数为 598/616/613/603/631/630/630/630，末尾 0.721 秒另有 1 个样本。
- `reads.errors=[]`；DB 在全部资源样本中恒为 294,764,544 bytes。RSS 样本 38,850,560→74,272,768 bytes（最小/最大 35,233,792/74,784,768；terminal 73,965,568）；CPU 1.75→4006.8125 process seconds。此为观测趋势，不诊断内存泄漏。
- 运行代码 SHA 只绑定上述旧 read-path；未重启、未热更新或回填。详尽可复核数值见[性能验证](performance-validation.md)。

此验证仅覆盖 SQLite 公开只读查询，不能推导全系统稳定性、写入一致性、Web 用户流、安全、生产负载、Gate/release 或客户验收。
