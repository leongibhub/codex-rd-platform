...
`--once` performs one polling/dispatch pass. Without it, the process stays foreground-resident until its host stop event is set. It reaps expired leases, limits active claims to `max_concurrency`, heartbeats owned leases, and asks the bounded owned process to cancel when the service stops, lifecycle pauses, or the lease is invalidated. Reconcile actual process state: a pause or OS signal is not evidence of successful cancellation. A pause invalidation, claimed-work control invalidation, claimed work invalidated by artifact/trace change, or expired lease that records `external_process_cancelled:false` fences that attempt across service instances; it is not dispatched again unless `safe_to_retry:true` is explicit. The dispatcher sends the observed work `version` with its claim, and Runtime checks that version plus the durable fence in the same SQLite transaction, so a pause/resume between polling and claim cannot authorize a stale argv launch. Once an explicit safe retry has claimed a later attempt, the historical unknown attempt does not permanently block a subsequent ordinary retry.

The host `work.claim` contract accepts optional `expected_version` and `safe_to_retry`. `expected_version`, when supplied, must be the exact positive integer read by the dispatcher; a changed work version rejects the claim atomically. `safe_to_retry` is an exact boolean and defaults to `false`. It is required for a claim fenced by an unknown external outcome; it is an explicit host decision, not evidence that a process was stopped. A `rollback` control refuses to create compensation while its original work is still `CLAIMED` or has an unresolved unknown-outcome fence. Reconcile the original outcome through the normal safe retry and completion path before requesting compensation; the adapter never asserts external cancellation on behalf of the host.
...
The recovery contract covers process interruption on a functioning filesystem. It does not promise durability under host power loss, faulty storage or a hostile same-OS-identity process; atomic replacement and fsynced file content alone do not prove those properties.
