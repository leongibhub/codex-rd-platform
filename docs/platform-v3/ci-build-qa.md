...
Date: 2026-09-06  
Status: `TASK-V3-012` retry unit/integration and `TASK-V3-013` revision-3 unit/integration are current and ready for review. Historical failed/superseded records remain below.
...
| TC-CI-IND-1206 | `TASK-V3-013` / `BUG-CI-003`; C++ adapter regression after Windows static GNU runtime linkage change | Actual public `stack-run` on this Windows host with native `cxx=NOT_AVAILABLE`, so the documented WSL adapter is selected | Run all declared C++ phases. Expect build PASS, unit output `5 tests passed`, integration output `3 integration checks passed`. This validates WSL fallback only; it cannot validate Windows native output or DLL loading. | PASS: `run-f57cbdee42a142c8a2f2bd8bf87c14b4`. |
...
| TASK-V3-013 revision-3 unit | `run-aca565e9486d425b8091808dddcbbec2` | Native-tool probe, Windows C++ linking, stack harness, and CI contract suites; 20 tests, exit 0, 1.685776s. | PASS |
| TASK-V3-013 revision-3 independent integration | `run-f57cbdee42a142c8a2f2bd8bf87c14b4` | Public `stack-run` for C++ manifest with `--root D:/codex-rd-platform`; build PASS, unit reports 5 tests, integration reports 3 checks, exit 0, 11.634032s. `cxx=NOT_AVAILABLE`, so adapter used WSL. | PASS / WSL only |

The current full independent suite directly qualified TC-1204/1205 again, 2/2 PASS in 0.348s, after revision 3. TC-1203 uses raw-byte capture and `mbcs` replacement decoding only for a failed `cmd /c mklink /J` diagnostic, avoiding unsafe UTF-8 decoding of Windows code-page output; it still requires the helper's explicit symlink/junction/reparse rejection. TC-1204 no longer requires a WSL distribution on a Windows CI runner: each CI OS runs the production resolver with its own Python and its own expected filename (`java.exe` on Windows; `java` on Linux). The direct qualification is supplemental to the current Task 012 and Task 013 Runtime phase evidence above.

## Explicit limits

- GitHub Actions online Ubuntu/Windows execution for revision 3, Windows native GNU DLL loading, and the `0xC0000139` recovery are **NOT_EXECUTED** until a real hosted workflow run and its evidence are read. The local revision-3 C++ run deliberately selected WSL because `stack-probe` returned `cxx=NOT_AVAILABLE`.
