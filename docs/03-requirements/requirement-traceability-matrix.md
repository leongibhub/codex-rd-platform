...
- `Defect Disposition: CLOSED` requires a typed `BUG-xxx`, the corrective commit/MR, `TC`, externally declared `EVD-xxx` regression evidence, `Verification Result: PASS`, `Evidence Status: VERIFIED`, and `Last Verified`; do not infer closure from a code change or a partial token such as `CLOSED TBD`.
