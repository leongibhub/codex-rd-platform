...
`Trace ID` must be a unique `TRACE-xxx` record identifier. Active linkage fields use typed references: `BG-xxx`, `PRD-xxx`, `REQ-xxx`/`NFR-xxx`, `DES-xxx`/`ADR-xxx`, `TASK-xxx`, `CR-xxx`, commit SHA or `MR-xxx`, `TC-xxx`, `EVD-xxx`, `BUG-xxx`, and `REL-xxx`. Ordinary prose does not satisfy a typed link.
