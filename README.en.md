...
| GitHub Actions CI | Frozen source `fbd9b36` passed [run 34095997725](https://github.com/leongibhub/codex-rd-platform/actions/runs/34095997725) at 2026-09-07 07:48:30 UTC: all 4/4 Windows/Ubuntu Runtime/platform and multistack-manifest jobs succeeded. Separate frozen local regressions were Runtime 307 (2 skips), platform 134 and independent 113 (1 skip), all OK; these are not CI test counts. The old `026263f` failure remains recorded. Later documentation commits are not claimed as covered by this source CI; CI does not establish a Gate, production deployment or acceptance. See the [CI execution record](docs/platform-v3/ci-execution.md). |
...
<a id="cr-v3-003-execution-endpoints-controlled-use-implemented-pending-final-review"></a>

## CR-V3-003 execution endpoints: controlled use
...
The endpoint and strict-orchestration module quality chains have completed independent testing and review; see the [delivery record](docs/platform-v3/completion-execution-delivery.md). Frozen local regressions ran 307 Runtime tests (2 skips), 134 platform tests and 113 independent tests (1 skip), all OK. The independent tester also executed all 11 declared phases of the five samples and their black-box regressions. These are not project acceptance: live Responses, real human approval, production deployment and native WeChat validation still require real external conditions. The failed live Codex smoke remains historical failure. See the [execution design](docs/platform-v3/completion-execution-design.md), [worker service](docs/platform-v3/worker-service.md), [approval provider](docs/platform-v3/approval-provider.md), [deployment executor](docs/platform-v3/deployment-executor.md), and [Linux setup](docs/platform-v3/linux-setup.md).
